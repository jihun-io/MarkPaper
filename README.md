...
### 2.1. 기능 상세
1. MarkPaper는 기본 확장자로 `*.mp`를 사용합니다. `*.mp` 파일 내부에 사진을 저장하기 때문에 자유롭게 사진 첨부가 가능합니다. `*.md` 파일로도 저장할 수 있지만, 파일에 사진이 저장되지 않습니다.
...
### 2.2. `*.mp` 파일 상세
MarkPaper는 사진이 포함된 마크다운 문서를 저장하기 위해 `.mp` 확장자를 사용합니다. `.mp` 파일은 ZIP 압축 포맷을 기반으로 작동하며, 파일 구조는 다음과 같습니다.
...
에디터에 사진을 드래그 앤 드롭하거나 '사진 추가' 버튼을 눌러 사진을 삽입하면 파일이 문서에 삽입됩니다. 첨부되는 사진은 참조할 때 파일 명 앞에 `$` 접두어가 붙습니다. (예시: `![예시 사진 1.jpg]($예시 사진 1.jpg)`)

`.mp` 파일을 저장할 때, 첨부했던 특정 사진이 마크다운 문서에서 참조되지 않으면 해당 사진 파일은 저장되지 않습니다. 
...
#### 5.1.1. 사진 삽입
`![사진 이름](사진 경로)`로 사진를 삽입할 수 있습니다. 웹 주소를 입력하여 파일을 웹에서 불러올 수 있으며, PC에 저장된 파일을 '사진 추가' 버튼 클릭 혹은 드래그 앤 드롭으로 삽입할 수도 있습니다. 마크다운으로 추가된 사진는 `style` 태그로 CSS에서 `img` 태그를 지정하여 스타일을 지정할 수 있지만, 개별적으로 인라인 스타일은 지정할 수 없습니다.
...
#### 5.2.1. 사진 삽입
MarkPaper는 `img` 태그로도 사진를 삽입할 수 있습니다. 마크다운 문법과는 달리 개별적으로 인라인 스타일을 지정할 수 있습니다. 
